...
interface GSTSettingsTableProps {
  searchTerm: string;
  status: string;
}

export default function GSTSettingsTable({ searchTerm, status }: GSTSettingsTableProps) {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      await fetchSettings(currentPage);
        ...
        <table className="w-full">
          <thead className="border-b border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-900/50">
            <tr>
              ...
              {(hasPermission("gst-settings", "view") ||
                hasPermission("gst-settings", "edit") ||
                hasPermission("gst-settings", "delete")) && (
                <th className="px-6 py-4 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">
                  Actions
                </th>
              )}
            ...
            {settings.length === 0 ? (
                  ...
                  <div className="mx-auto max-w-md">
                    ...
                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                      {searchTerm || status !== "all"
                        ...
                        : "Get started by creating a new GST setting."}
                    </p>
                  </div>
                </td>
              </tr>
              ...
              settings.map((setting) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <span className="inline-flex rounded-full bg-green-100 px-3 py-1 text-sm font-semibold text-green-800 dark:bg-green-900/30 dark:text-green-400">
                      {formatPercentage(setting.percentage)}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <span className="text-sm text-gray-700 dark:text-gray-300">
                      {setting.state_ids || "All States"}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <span className="text-sm text-gray-700 dark:text-gray-300">
                      {setting.country_ids || "All Countries"}
                    </span>
                  </td>
                  <td className="px-6 py-4">{getStatusBadge(setting.is_active)}</td>
                  {(hasPermission("gst-settings", "view") ||
                    hasPermission("gst-settings", "edit") ||
                    hasPermission("gst-settings", "delete")) && (
                    <td className="px-6 py-4">
