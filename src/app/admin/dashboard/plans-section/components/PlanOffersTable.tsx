...
interface PlanOffer {
  ...
  validity_hours_after_display: number;
  is_special_offer: number;
  is_extended_offer: number;
  is_active: number;
  created_at: string;
  updated_at: string;
}
...
interface PlanOffersTableProps {
  searchTerm: string;
  status: string;
  offerType: string;
}

export default function PlanOffersTable({ searchTerm, status, offerType }: PlanOffersTableProps) {
  ...
  const fetchOffers = useCallback(
    async (page = 1) => {
      try {
        ...
        if (offerType && offerType !== "all") {
          params.append("discountType", offerType);
        }
        ...
        if (accessToken) {
          ...
          if (response.ok) {
            const data = await response.json();
            // Enrich offers with plan names
            const enrichedOffers = data.data.data.map((offer: PlanOffer) => ({
              ...offer,
              plan_name: plans.find(p => p.id === offer.plan_id)?.name || `Plan #${offer.plan_id}`,
            }));
    ...
    [searchTerm, status, offerType, itemsPerPage, plans]
  );
  ...
  const getSpecialOfferBadge = (isSpecial: number) => {
    if (isSpecial === 1) {
      return (
        <span className="inline-flex rounded-full bg-orange-100 px-3 py-1 text-xs font-semibold text-orange-800 dark:bg-orange-900/30 dark:text-orange-400">
          Special Offer
        </span>
      );
    }
  ...
  const getExtendedOfferBadge = (isExtended: number) => {
    if (isExtended === 1) {
      return (
        <span className="inline-flex rounded-full bg-teal-100 px-3 py-1 text-xs font-semibold text-teal-800 dark:bg-teal-900/30 dark:text-teal-400">
          Extended Offer
        </span>
      );
    }
        ...
        <table className="w-full">
          <thead className="border-b border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-900/50">
            <tr>
              ...
              {(hasPermission("plan-offers", "view") ||
                hasPermission("plan-offers", "edit") ||
                hasPermission("plan-offers", "delete")) && (
                <th className="px-6 py-4 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">
                  Actions
                </th>
              )}
                ...
                <td colSpan={7} className="px-6 py-12 text-center">
                  <div className="mx-auto max-w-md">
                    ...
                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                      {searchTerm || status !== "all" || offerType !== "all"
                ...
                >
                  <td className="px-6 py-4">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {getSpecialOfferBadge(offer.is_special_offer)}
                        {getExtendedOfferBadge(offer.is_extended_offer)}
                      </div>
                      {offer.description && (
                  ...
                  <td className="px-6 py-4">
                    <div className="text-sm text-gray-700 dark:text-gray-300">
                      {offer.applicable_duration_months.map(month => `${month}m`).join(", ")}
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="text-sm text-gray-700 dark:text-gray-300">
                    ...
                    <div className="text-xs text-gray-500">
                      Valid for {offer.validity_hours_after_display}h
                    </div>
                  </td>
                  <td className="px-6 py-4">{getStatusBadge(offer.is_active)}</td>
                  {(hasPermission("plan-offers", "view") ||
                    hasPermission("plan-offers", "edit") ||
                    hasPermission("plan-offers", "delete")) && (
                    <td className="px-6 py-4">
