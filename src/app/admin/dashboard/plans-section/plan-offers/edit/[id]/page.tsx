...
export default function EditPlanOfferPage() {
  ...
  useEffect(() => {
    const fetchOffer = async () => {
      try {
        ...
        const response = await fetch(`${API_URL}/plan-offers/${id}?platformId=${platformId}`, {
          headers: { Authorization: `Bearer ${accessToken}` },
        });
        ...
        setFormData({
          ...
          applicableDurationIds: result.data.applicableDurationIds || [],
          ...
          conflictHandleDiscount: result.data.conflictHandleDiscount,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/plan-offers/${id}`, {
        method: "PUT",
        headers: {
      ...
      await showSuccess("Plan offer updated successfully");
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <PageHeader
        title="Edit Plan Offer"
        // description="Update offer details and configurations"
        breadcrumbItems={[
        ...
        actionButtons={[
          ...
          {
            label: isLoading ? "Updating..." : "Update Offer",
            type: "submit",
            form: "offer-form",
            variant: "primary",
            disabled: isLoading,
