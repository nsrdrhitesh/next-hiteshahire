...
export default function PlansPage() {
  ...
  const statuses = [
    ...
    { id: "1", name: "Active Plans" },
    { id: "0", name: "Inactive Plans" },
  ];
  ...
  const fetchPlans = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        ...
        setTotalPlans(data.data.meta.total);
      } else {
  ...
  }, [currentPage, searchTerm, selectedStatus, platformId]);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const accessToken = localStorage.getItem("access_token");
      const response = await fetch(`${API_URL}/plans/${selectedPlanId}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
  ...
  const endIndex = Math.min(currentPage * itemsPerPage, totalPlans);
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        ...
        createButton={{
          href: "/admin/dashboard/plans-section/plans/create",
          label: "Create New Plan",
          permission: { resource: "plans", action: "create" }
        }}
          ...
          <table className="w-full">
            <thead className="border-b border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-900/50">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Slug</th>
                <th className="px-6 py-4 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Monthly Price</th>
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center">
                    <div className="mx-auto max-w-md">
                      ...
                      <h3 className="mt-2 text-sm font-semibold text-gray-900 dark:text-white">No plans found</h3>
                      <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Get started by creating a new plan.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                plans.map((plan) => (
                  <tr key={plan.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    <td className="px-6 py-4">
                      <div>
                        <h3 className="text-sm font-medium text-gray-900 dark:text-white">{plan.name}</h3>
                        {plan.description && (
                          <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">{plan.description.substring(0, 80)}...</p>
                        )}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <span className="font-semibold text-gray-900 dark:text-white">{formatPrice(plan.monthlyPrice)}</span>
                      <span className="text-xs text-gray-500 dark:text-gray-400">/month</span>
                    </td>
                    <td className="px-6 py-4">
                      <span className="text-sm text-gray-700 dark:text-gray-300">{plan.sortOrder}</span>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <Link
                          href={`/admin/dashboard/plans-section/plans/edit/${plan.id}`}
