...
export default function DomainSeoCodesTable({ searchTerm }: Props) {
  const API_URL = process.env.NEXT_PUBLIC_BACKEND_DATA_URL || "";
  const [currentPage, setCurrentPage] = useState(1);
  const [codes, setCodes] = useState<DomainSeoCode[]>([]);
  ...
  const fetchData = useCallback(
    async (page = 1) => {
      ...
      try {
        ...
        const res = await fetch(`${API_URL}/domain/seo-codes/${selectedPlatformId}?${params}`, {
          headers: { Authorization: `Bearer ${token}` },
        });

        if (!res.ok) throw new Error("Failed to fetch");

        const json: ApiResponse<DomainSeoCode> = await res.json();

        setCodes(json.data.data);
  ...
  const handleDelete = async () => {
    ...
    try {
      const token = localStorage.getItem("access_token");
      const res = await fetch(`${API_URL}/domain/seo-codes/${selectedId}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
    ...
    return (
      <div className="flex items-center justify-center rounded-xl bg-white p-12 shadow-sm dark:bg-gray-800">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">Loading tracking codes...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="overflow-hidden rounded-xl bg-white shadow-sm dark:bg-gray-800">
        ...
        <table className="w-full min-w-max">
          <thead className="border-b border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-900/50">
            <tr>
              ...
              {(hasPermission("domain-seo-codes", "view") ||
                hasPermission("domain-seo-codes", "edit") ||
                hasPermission("domain-seo-codes", "delete")) && (
                <th className="px-6 py-4 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">
                  Actions
                </th>
              )}
                ...
                <td colSpan={6} className="px-6 py-12 text-center">
                  <div className="mx-auto max-w-md">
                    ...
                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                      {searchTerm ? "Try a different search term" : "Add your first pixel / tag code."}
                    </p>
                  </div>
                </td>
              </tr>
                ...
                <tr key={code.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  ...
                  <td className="px-6 py-4">
                    <span className="inline-flex items-center rounded-md bg-purple-50 px-2 py-1 text-xs font-medium text-purple-700 ring-1 ring-inset ring-purple-700/10 dark:bg-purple-900/30 dark:text-purple-300">
                      {ACCOUNT_TYPE_LABELS[code.accountType] || "Unknown"}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <span className="text-sm text-gray-700 dark:text-gray-300">
                      {POSITION_TYPE_LABELS[code.positionType] || "—"}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <div className="max-w-md truncate text-sm text-gray-500 dark:text-gray-400">
                      {code.accountCode.substring(0, 80)}
                      {code.accountCode.length > 80 ? "..." : ""}
                    </div>
                  </td>
                  ...
                  {(hasPermission("domain-seo-codes", "view") ||
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        {hasPermission("domain-seo-codes", "view") && (
                          <Link
                            href={`/admin/dashboard/domain-manage/seo-codes/view/${code.id}`}
                        ...
                        {hasPermission("domain-seo-codes", "edit") && (
                          <Link
                            href={`/admin/dashboard/domain-manage/seo-codes/edit/${code.id}`}
      ...
      {/* Pagination – exact same style as branding */}
      {total > 0 && (
