...
export default function DomainPaymentConfigTable({ searchTerm }: Props) {
  const API_URL = process.env.NEXT_PUBLIC_BACKEND_DATA_URL || "";
  const [currentPage, setCurrentPage] = useState(1);
  const [configs, setConfigs] = useState<DomainPaymentConfig[]>([]);
  ...
  const fetchData = useCallback(async (page = 1) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/domain/payment-config/${selectedPlatformId}?${params}`, {
        headers: { Authorization: `Bearer ${token}` },
      });

      if (!res.ok) throw new Error("Failed to fetch payment configs");

      const json: ApiResponse<DomainPaymentConfig> = await res.json();

      setConfigs(json.data.data);
  ...
  const handleDelete = async () => {
    ...
    try {
      const token = localStorage.getItem("access_token");
      const res = await fetch(`${API_URL}/domain/payment-config/${selectedId}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const modeLabel = (mode: number) => (mode === 1 ? "Test" : mode === 2 ? "Live" : "—");
  const statusLabel = (status: number) => (status === 1 ? "Active" : status === 2 ? "Inactive" : "—");
    ...
    return (
      <div className="flex items-center justify-center rounded-xl bg-white p-12 shadow-sm dark:bg-gray-800">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">Loading payment configurations...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="overflow-hidden rounded-xl bg-white shadow-sm dark:bg-gray-800">
        ...
        <table className="w-full min-w-max">
          <thead className="border-b border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-900/50">
            <tr>
              ...
              {(hasPermission("domain-payment-config", "view") ||
                hasPermission("domain-payment-config", "edit") ||
                hasPermission("domain-payment-config", "delete")) && (
                <th className="px-6 py-4 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">
                  Actions
                </th>
              )}
              ...
              <tr>
                <td colSpan={6} className="px-6 py-12 text-center">
                  <div className="mx-auto max-w-md">
                    ...
                    <h3 className="mt-2 text-sm font-semibold text-gray-900 dark:text-white">No payment configs found</h3>
                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                      {searchTerm ? "Try a different search term" : "Create your first payment gateway configuration."}
                    </p>
                  </div>
                </td>
              </tr>
              ...
              configs.map((c) => (
                <tr key={c.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  <td className="px-6 py-4">
                    <div className="text-sm font-medium text-gray-900 dark:text-white">{c.gatewayName}</div>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${
                      c.mode === 1 ? "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300" :
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${
                      c.status === 1 ? "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300" :
                  ...
                  {(hasPermission("domain-payment-config", "view") ||
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        {hasPermission("domain-payment-config", "view") && (
                          <Link
                            href={`/admin/dashboard/domain-manage/payment-config/view/${c.id}`}
                        ...
                        {hasPermission("domain-payment-config", "edit") && (
                          <Link
                            href={`/admin/dashboard/domain-manage/payment-config/edit/${c.id}`}
      ...
      {/* Pagination - same as your DomainBrandingTable */}
      {total > 0 && (
