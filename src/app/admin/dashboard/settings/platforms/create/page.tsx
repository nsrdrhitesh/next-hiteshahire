...
export default function CreatePlatformPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    code: "",
    description: "",
    is_active: true, // This is boolean
  ...
  }, [formData.religion_ids, religions]);
  ...
  useEffect(() => {
    ...
    if (countries.length > 0 && formData.country_ids.length > 0) {
      fetchStateGroups();
    }
  }, [formData.country_ids, countries]);
  ...
  const validateForm = () => {
    ...
    if (!formData.code) newErrors.code = "Platform code is required";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Create payload with boolean is_active
      const payload = {
        name: formData.name,
        code: formData.code,
      ...
      if (!response.ok) {
        if (responseData.message && typeof responseData.message === "object") {
          ...
          setError(responseData.message || "Create failed");
        }
        return;
      }

      await showSuccess("Platform created successfully");
