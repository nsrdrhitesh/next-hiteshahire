...
export default function PlatformTable({ searchTerm, status }: PlatformTableProps) {
  ...
  // Fetch platforms from API with pagination
    ...
    try {
      ...
      if (accessToken) {
        ...
        if (response.ok) {
          ...
          // console.log("Fetch Blogs Data Here 01 :- ", data);

          // Transform API data
          const transformedPlatforms: Platform[] = data.data.data.map(
          ...
          setTotalPlatforms(data.data.meta.total);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/platforms/${selectedPlatformId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${accessToken}`,
      ...
      await fetchPlatforms(currentPage);
  ...
  useEffect(() => {
    fetchPlatforms(1); // Always start from page 1 when filters change
  }, [fetchPlatforms]);
  ...
  const endIndex = Math.min(currentPage * itemsPerPage, totalPlatforms);
    ...
    <div className="overflow-hidden rounded-xl bg-white shadow-sm dark:bg-gray-800">
            ...
            {platforms.length === 0 ? (
              ...
              platforms.map((platform) => (
                ...
                >
                  ...
                  {/* <td className="px-6 py-4">{getStatusBadge(platform.is_active)}</td> */}
                   {(hasPermission("platforms", "view") ||
                    hasPermission("platforms", "edit") ||
                    hasPermission("platforms", "delete")) && (
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      {hasPermission("platforms", "view") && (
                      <Link
                        href={`/admin/dashboard/settings/platforms/view/${platform.id}`}
                      ...
                      {hasPermission("platforms", "edit") && (
                      <Link
                        href={`/admin/dashboard/settings/platforms/edit/${platform.id}`}
      ...
      <div className="flex items-center justify-between border-t border-gray-200 px-6 py-4 dark:border-gray-700">
        <div className="text-sm text-gray-700 dark:text-gray-400">
          ...
          <span className="font-medium">{totalPlatforms}</span> results
        </div>
